...
def train(x, y, lookback, modelname, nunits=40, epochs=200):
    ...
    model.add(Conv1D(64, 3))
    model.add(MaxPool1D(2))
    model.add(Bidirectional(tf.keras.layers.LSTM(nunits)))
    ...
    history = model.fit(x, y, epochs=epochs,
                        callbacks=[SaveCallback(modelname)])
...
def init_argparse():
    headline = "Gerador de Augusto dos Anjos. Uses LSTM's, "
    "which are not ideal for data generation"
    parser = argparse.ArgumentParser(description=headline)
    parser.add_argument('-gp', '--generate-poem', type=int,
...
if __name__ == '__main__':
    ...
    # h, m = train(x,y, 80, epochs=10)
    #model = tf.keras.models.load_model('Modelos/modelo50.h5')
